...
Apps are tools like `FastApi`, `PgVector`, `Streamlit`, `Jupyter`, `Django` that we define as python classes and run using `ethosian start` or `ethosian ws up`.

When running Apps using ethosian, think of them as infrastructure as code but at a higher level of abstraction. Instead of defining containers, volumes etc. we define the application we want to run. We run **Applications as Code** instead of Infrastructure as Code.
...
Copy the following contents to a file `resources.py` and run `ethosian start resources.py`

```python resources.py
from ethosian.docker.app.jupyter import Jupyter
...
Defining **Applications as Code** also allows us to package "software systems" into templates. Meaning every ethosian template can run locally using docker or on AWS with 1 command.
