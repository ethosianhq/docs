...
```bash terminal
ethosian ws up --env dev --infra docker --type image --force
```

```bash short options
ethosian ws up -e dev -i docker -t image -f
```
...
```bash full options
ethosian ws up --env dev --infra docker --force
```
...
```bash short options
ethosian ws up -e dev -i docker -f
```
