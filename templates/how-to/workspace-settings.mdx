...
Setting `build_images=True` will build images locally when running `ethosian ws up dev:docker` or `ethosian ws up prd:docker`.
...
Setting `push_images=True` will push images after building when running `ethosian ws up dev:docker` or `ethosian ws up prd:docker`.
