...
Your application uses the `ethosian` images by default. To use your own image:
...
- If using dockerhub it would be something like `ethosian`.
...
```bash terminal
ethosian ws up --env prd --infra docker --type image --force
```

```bash shorthand
ethosian ws up -e prd -i docker -t image -f
```
...
```bash Build Images
ethosian ws up prd:docker
```

```bash Force Build Images
ethosian ws up prd:docker -f
```
...
```bash terminal
ethosian ws patch --env prd --infra aws --name td
```

```bash shorthand
ethosian ws patch -e prd -i aws -n td
```
