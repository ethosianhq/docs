...
```bash Mac
ethosian ws create -t llm-os -n llm-os
```
...
└── workspace               # ethosian workspace directory
    ...
    └── settings.py         # ethosian workspace settings
```
...
<Steps>
  ...
  <Step title="Start FastApi">
    ...
    ```bash terminal
    ethosian ws up --group api
    ```

    ```bash shorthand
    ethosian ws up dev:docker:api
    ```
